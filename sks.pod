...
Set the confguration directory with the files sksconf, membership and mailsync.
...
Set the KDB database directory path.

=item -ptree_dbdir

Set the PTree database directory path.

=item -webdir

Set the web server directory path.

=item -logdir

Set the location of the logfiles directory.

=item -rundir

Set the location for the storage of transient state files.

=item -diffdir

Set the directory path where the key differences are dumped to.

=item -dumpdir

Set the directory path to the keydump files.
